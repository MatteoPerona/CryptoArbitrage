...
def high():
    ...
    if abs(min) > max:
        ...
        return [BTCMarket[mnC][0:3], min]
    print(f'coin is: {BTCMarket[mxC][0:3]}/USDT, discrepancy is: ${max}')
    return [BTCMarket[mxC][0:3], max]
...
def arbitrage():
    ...
    temp = cash+discrepancyNow-totFee
    if temp <= cash:
        print(f'would have lost ${temp-cash}')
        with open(wallet, 'a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([time_now, listvar[0], temp])
        return
    cash = temp
    with open(wallet, 'a', newline='') as file:
        writer = csv.writer(file)
    ...
    try:
        while True:
            ...
            if verbose:
                ...
                print(f'You made: ${cash-oldCash} and now have ${cash}')
